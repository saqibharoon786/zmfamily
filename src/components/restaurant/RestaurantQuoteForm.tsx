...
export function RestaurantQuoteForm() {
  const [pending, startTransition] = useTransition();
  const [mealCategory, setMealCategory] = useState("Executive Office Meals");
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const deliveryDate = String(data.get("date") ?? "").trim();
    const portionCount = String(data.get("portions") ?? "").trim();
      ...
      try {
        const response = await fetch(FORM_ENDPOINT, {
          ...
          body: JSON.stringify({
            ...
            "Portion / Meal Count": portionCount,
            "Meal Category": mealCategory,
            "Order Notes & Dietary Preferences": details,
            _subject: `Restaurant / Food Order Request from ${name} — Z M FAMILY`,
            _template: "table",
            _replyto: email,
            _captcha: "false",
          }),
        ...
        if (!response.ok) {
          throw new Error(result.message ?? "Could not send your food order request.");
        }

        toast.success(
          "Thank you! Your restaurant & kitchen order inquiry was sent. Our chef team will contact you shortly.",
        );
        form.reset();
        setMealCategory("Executive Office Meals");
      } catch {
    ...
    >
      <div className="border-b border-border/80 pb-4">
        <h3 className="font-display text-2xl font-light text-foreground">
          Restaurant & Food Inquiry Form
        </h3>
        <p className="mt-1 text-xs text-muted-foreground">
          Order daily office meals, party platters, or gourmet kitchen dishes in Qatar.
        </p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <input
            name="location"
            type="text"
            placeholder="West Bay, Pearl, Al Khor..."
            className={inputClass}
          />
        </div>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          <label className={labelClass}>Portions / Person Count</label>
          <input
            name="portions"
            type="text"
            placeholder="e.g. 10 meals, 50 persons..."
            className={inputClass}
          />
        </div>
      </div>
      ...
      <div>
        <label className={labelClass}>Dining & Food Category</label>
        <div className="flex flex-wrap gap-2.5">
          {[
            "Executive Office Meals",
      ...
      <div>
        <label className={labelClass}>Order Notes & Dietary Preferences</label>
        <textarea
          name="details"
          rows={3}
          placeholder="Cuisine style (Arabic, Asian, Continental), dietary restrictions, delivery timing..."
      ...
      <p className="text-center text-[0.7rem] font-medium text-muted-foreground">
        Direct response from Z M FAMILY Kitchen Team · Fast Response
      </p>
    </form>
  );
}
